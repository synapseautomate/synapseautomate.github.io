...
<style>
:root{--bg:#06111f;--panel:#0d2033;--panel2:#102a40;--text:#edf7fb;--muted:#9fb4c2;--line:#24475f;--cyan:#47ded2;--blue:#4da3ff;--danger:#ff8d8d}
*{box-sizing:border-box}
html{scroll-behavior:smooth}
body{margin:0;background:linear-gradient(180deg,#06111f,#071827 45%,#06111f);color:var(--text);font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;line-height:1.65}
a{color:#82dfee;text-decoration:none} a:hover{text-decoration:underline}
.container{width:min(1120px,calc(100% - 32px));margin:auto}
.site-header{position:sticky;top:0;z-index:10;background:rgba(6,17,31,.94);border-bottom:1px solid var(--line);backdrop-filter:blur(10px)}
.nav{min-height:72px;display:flex;align-items:center;gap:22px}.brand{font-weight:800;letter-spacing:.02em;color:#fff;font-size:1.15rem}
.nav-links{display:flex;gap:18px;margin-left:auto}.nav-cta,.btn{display:inline-flex;align-items:center;justify-content:center;padding:12px 18px;border-radius:999px;background:linear-gradient(90deg,var(--cyan),#35b8df);color:#04121f;font-weight:800}
.menu-button{display:none}.skip{position:absolute;left:-9999px}
.breadcrumb{padding:22px 0;color:var(--muted);font-size:.92rem}
.page-hero{padding:56px 0 36px}.page-hero h1{font-size:clamp(2.1rem,5vw,4rem);line-height:1.05;margin:.35em 0}.lead{max-width:900px;font-size:1.15rem;color:#c6d8e2}
.kicker{color:var(--cyan);text-transform:uppercase;font-weight:800;letter-spacing:.12em;font-size:.8rem}
.section,.section-tight{padding:54px 0}.section-tight{padding-top:28px}.section-head{display:flex;justify-content:space-between;gap:24px;align-items:end;margin-bottom:24px}.section-head h2{margin:.2em 0;font-size:clamp(1.5rem,3vw,2.3rem)}.section-head p{max-width:560px;color:var(--muted)}
...
.card{background:linear-gradient(180deg,var(--panel2),var(--panel));border:1px solid var(--line);border-radius:18px;padding:22px}.card h3{margin-top:0}.card li{margin:.5rem 0}
.table-wrap{overflow:auto;border:1px solid var(--line);border-radius:18px}table{border-collapse:collapse;width:100%;min-width:880px;background:var(--panel)}th,td{text-align:left;padding:15px;border-bottom:1px solid var(--line);vertical-align:top}th{color:var(--cyan);background:#0b1c2d}
.steps{display:grid;gap:12px;padding-left:1.25rem}.steps li{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:14px 16px}
.band{border:1px solid var(--line);border-radius:22px;padding:26px;background:linear-gradient(120deg,#0d2439,#0b1b2d)}.band-grid{display:flex;justify-content:space-between;align-items:center;gap:24px}
.footer{border-top:1px solid var(--line);margin-top:38px;padding:38px 0;color:var(--muted)}.footer-grid{display:grid;grid-template-columns:2fr 1fr 1fr 1fr;gap:22px}.footer h4{color:#fff}.footer a{display:block;margin:.35rem 0}.copyright{border-top:1px solid var(--line);margin-top:28px;padding-top:18px;display:flex;justify-content:space-between;gap:20px;font-size:.9rem}
@media(max-width:760px){.nav-links{display:none}.nav{justify-content:space-between}.grid-2,.grid-3,.footer-grid{grid-template-columns:1fr}.section-head,.band-grid,.copyright{display:block}.nav-cta{font-size:.85rem}.page-hero{padding-top:34px}.section,.section-tight{padding:36px 0}}
</style>
...
<a class="brand" href="../index.html" aria-label="Synapse Automate ana sayfa">Synapse Automate</a>
...
<div><a class="brand" href="../index.html">Synapse Automate</a><p>Kurumsal AI otomasyonu, AI-native ürünler ve insan denetimli dijital operasyon sistemleri.</p></div>
...
for rel in changed:
    print("UPDATED:", rel)
print("KILORY PRE-RELEASE PROOF + CLAIM SAFETY QA V2: PASS")
